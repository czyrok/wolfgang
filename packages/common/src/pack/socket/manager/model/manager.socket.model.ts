...
export class ManagerSocketModel implements BuildManagerSocketInterface, AlternativeBuildManagerSocketInterface {
    private _socketIoManager: Manager

    private _storage: StorageMetadataManagerSocketModel = new StorageMetadataManagerSocketModel(this)

    public constructor(
        url: string,
        port: number
    ) {
        this._socketIoManager = new Manager(`${url}:${port}`, {
            autoConnect: false,
    ...
    buildNamespace(namespaceName: string): NamespaceSocketModel {
        const namespace: NamespaceSocketModel
            = new NamespaceSocketModel(this, this.storage.getMetadata(namespaceName), namespaceName)
    ...
    buildLink<S, R, E = any>(namespaceName: string, event: string): LinkNamespaceSocketModel<S, R, E> {
        const link: LinkNamespaceSocketModel<S, R, E>
            = new LinkNamespaceSocketModel(this.storage.getMetadata(namespaceName), event)

        return link
    }

    buildBaseLink<S, R, E = any>(event: string): LinkNamespaceSocketModel<S, R, E> {
        const link: LinkNamespaceSocketModel<S, R, E> = new LinkNamespaceSocketModel(this.storage.getMetadata(''), event)
