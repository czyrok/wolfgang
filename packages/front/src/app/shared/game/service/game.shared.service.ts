    ...
    AlternativeBuildManagerSocketInterface
{
  ...
  public get socketManager(): ManagerSocketModel {
    if (!this._socketManager) {
      this._socketManager = new ManagerSocketModel(environment.GAME_URL);
      this._socketManager.socketIoManager.opts.path =
        environment.GAME_PATH_SOCKET_IO;
  ...
  private get socketNamespace(): NamespaceSocketModel {
    if (!this._socketNamespace)
      this._socketNamespace = this.socketManager.buildNamespace(
   ...
   */
  private set alreadyInGameAlert(
    value: DisplayAlertSharedInterface | undefined
  ) {
  ...
  public async checkStatus(): Promise<void> {
    const checkUserGameLink: LinkNamespaceSocketModel<void, string> =
      await this.socketSharedService.buildLink('/game', 'checkUserGame');

    return new Promise(
      (resolve: (value: void) => void, reject: (error: any) => void) => {
  ...
  public async joinGameAsPlayer(): Promise<boolean> {
    ...
    const joinLink: LinkNamespaceSocketModel<void, boolean> =
      await this.buildBaseLink('join');
  ...
  private async checkParty(gameId: string): Promise<boolean> {
    return this.socketSharedService.check<undefined>(
      '/play/' + gameId,
      'check',
      undefined
    );
  ...
  public displayJoinYourGameAlert(): void {
    ...
    if (this.inGame) {
      this.alreadyInGameAlert = this.displayAlertSharedService.emitInform(
